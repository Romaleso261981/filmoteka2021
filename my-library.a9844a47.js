...
r.myLibraryBtn.addEventListener("click",i),
...
async function(e){//!
if(console.log("Вешаю слушателя на открытие МОДАЛКИ (onMovieDetails)"),!e.target.closest("li"))return;{const o=e.target.closest("li");c=Number(o.getAttribute("key")),async function(e){JSON.parse(localStorage.getItem("watched")).find((o=>{if(console.log(o),o.id===e)return t=o})),JSON.parse(localStorage.getItem("queue")).find((o=>{if(o.id===e)return t=o}))}(c),console.log("idFilms:",c)}let t={};console.log(t),
//! ПОКАЗЫВАЕМ Spinner
...
r.watchedHeader.addEventListener("click",i),r.queueHeader.addEventListener("click",(function(){console.log("Вешаю слушателя на кнопку my-library.js==>QUEUE"),//!
...
JSON.parse(localStorage.getItem("watched")),JSON.parse(localStorage.getItem("queue"));function i(){
...
//! --------------------------------------------------------------------------------------------
function(e){return e.map((({id:e,poster_path:t,title:o,name:n,genres:a,first_air_date:r,release_date:c,vote_average:l})=>{const i=a.map((e=>e.name)).join(", "),d=(r||c||"???? - ?? - ??").substr(0,4),s=l.toFixed(1);let u=o;o&&(u=o.toUpperCase());let m=n;if(n){n.toUpperCase();//!!! тут ошибка сделана СПЕЦИАЛЬНО!!!
