var e="undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:"undefined"!=typeof window?window:"undefined"!=typeof global?global:{},t={},a={},o=e.parcelRequirea6b3;null==o&&((o=function(e){if(e in t)return t[e].exports;if(e in a){var o=a[e];delete a[e];var n={id:e,exports:{}};return t[e]=n,o.call(n.exports,n,n.exports),n.exports}var d=new Error("Cannot find module '"+e+"'");throw d.code="MODULE_NOT_FOUND",d}).register=function(e,t){a[e]=t},e.parcelRequirea6b3=o);var n=o("3oXzi");o("gY4QW");var d=o("bGAxz");
...
n.default.startSpinner(),"watched"===s&&(c=r.find((e=>e.id===i)));"queue"===s&&(c=u.find((e=>e.id===i)));
//! ==> Открываем модалку
...
l.watchedHeader.addEventListener("click",f),l.queueHeader.addEventListener("click",(function(){
//! Назначаем тип станицы QUEUE для логики работы кнопок МОДАЛКИ
s="queue",
//! Очищаем контейнер:
p();
//! Рисование интерфейса
w(JSON.parse(localStorage.getItem("queue"))??[])}
...
let i=1,c=null,s="",r=JSON.parse(localStorage.getItem("watched"))??[],u=JSON.parse(localStorage.getItem("queue"))??[];
...
if(r.find((e=>e.id===c.id)))return Notiflix.Notify.warning(`Фильм ${c.title||c.name} уже есть в WATCHED`,{position:"center-top",clickToClose:!0,timeout:2500}),l.watchedModal.textContent="DELETE FROM WATCHED",l.watchedModal.classList.contains("modal-button-add-watched")&&l.watchedModal.classList.remove("modal-button-add-watched"),void(l.watchedModal.classList.contains("modal-button-delete")||l.watchedModal.classList.add("modal-button-delete"));
//! Запись фильма в localStorage
r=[...r,c],localStorage.setItem("watched",JSON.stringify(r)),Notiflix.Notify.success(`Фильм ${c.title||c.name} добавлен в WATCHED`,{position:"center-top",clickToClose:!0,timeout:2500}),
//! Смена названия (textContent) кнопки на "DELETE FROM WATCHED"
l.watchedModal.textContent="DELETE FROM WATCHED",l.watchedModal.classList.contains("modal-button-add-watched")&&l.watchedModal.classList.remove("modal-button-add-watched"),l.watchedModal.classList.contains("modal-button-delete")||l.watchedModal.classList.add("modal-button-delete")}else"DELETE FROM WATCHED"===e&&(r=r.filter((e=>e.id!==c.id)),localStorage.setItem("watched",JSON.stringify(r)),Notiflix.Notify.info(`Фильм ${c.title||c.name} удален из WATCHED`,{position:"center-top",clickToClose:!0,timeout:2500}),l.watchedModal.textContent="ADD TO WATCHED",l.watchedModal.classList.contains("modal-button-delete")&&l.watchedModal.classList.remove("modal-button-delete"),l.watchedModal.classList.contains("modal-button-add-watched")||l.watchedModal.classList.add("modal-button-add-watched"),"watched"===s&&(y(),
//! Очищаем контейнер:
p(),w(r)))}
//! +++ Запрос полной информации о фильме для МОДАЛКИ +++
function b(){const e=l.queueModal.textContent;if("ADD TO QUEUE"===e){
//! Блокировка повторной записи фильма в localStorage (ВРЕМЕННО)
if(u.find((e=>e.id===c.id)))return Notiflix.Notify.warning(`Фильм ${c.title||c.name} уже есть в QUEUE`,{position:"center-top",clickToClose:!0,timeout:2500}),l.queueModal.textContent="DELETE FROM QUEUE",l.queueModal.classList.contains("modal-button-add-queue")&&l.queueModal.classList.remove("modal-button-add-queue"),void(l.queueModal.classList.contains("modal-button-delete")||l.queueModal.classList.add("modal-button-delete"));
//! Запись фильма в localStorage
u=[...u,c],localStorage.setItem("queue",JSON.stringify(u)),Notiflix.Notify.success(`Фильм ${c.title||c.name} добавлен в QUEUE`,{position:"center-top",clickToClose:!0,timeout:2500}),
//! Смена названия (textContent) кнопки на "DELETE FROM QUEUE"
l.queueModal.textContent="DELETE FROM QUEUE",l.queueModal.classList.contains("modal-button-add-queue")&&l.queueModal.classList.remove("modal-button-add-queue"),l.queueModal.classList.contains("modal-button-delete")||l.queueModal.classList.add("modal-button-delete")}else"DELETE FROM QUEUE"===e&&(u=u.filter((e=>e.id!==c.id)),localStorage.setItem("queue",JSON.stringify(u)),Notiflix.Notify.info(`Фильм ${c.title||c.name} удален из QUEUE`,{position:"center-top",clickToClose:!0,timeout:2500}),l.queueModal.textContent="ADD TO QUEUE",l.queueModal.classList.contains("modal-button-delete")&&l.queueModal.classList.remove("modal-button-delete"),l.queueModal.classList.contains("modal-button-add-queue")||l.queueModal.classList.add("modal-button-add-queue"),"queue"===s&&(y(),
//! Очищаем контейнер:
p(),w(u)))}function f(){s="watched",
//! Очищаем контейнер:
p();
//! Рисование интерфейса
w(JSON.parse(localStorage.getItem("watched"))??[])}
//! ++++++++++++++ Ф-ция, к-рая очищает контейнер при новом вводе данных в input form: ++++++++++++++
function p(){l.moviesCards.innerHTML=""}
