...
i.closeModalBtn.addEventListener("click",v),i.backdrop.addEventListener("click",(function(e){e.currentTarget===e.target&&v()})),
...
i.watchedHeader.addEventListener("click",h),i.queueHeader.addEventListener("click",(function(){console.log("Вешаю слушателя на кнопку my-library.js==>QUEUE"),//!
...
let u=1,m=null,M="",E=JSON.parse(localStorage.getItem("watched"))??[],f=JSON.parse(localStorage.getItem("queue"))??[];
//! Создаем глобальную переменную (idFilms) для хранения idF одного фильма
function g(){console.log("Вешаю слушателя на кнопку ADD TO WATCHED в МОДАЛКЕ"),//!
...
//! Блокировка повторной записи фильма в localStorage (ВРЕМЕННО)
if(f.find((e=>e.id===m.id)))return e(n).Notify.warning(`Фильм ${m.title||m.name} уже есть в QUEUE`,{timeout:3500}),i.queueModal.textContent="DELETE FROM QUEUE",i.queueModal.classList.contains("colorGreen")&&i.queueModal.classList.remove("colorGreen"),void(i.queueModal.classList.contains("colorRed")||i.queueModal.classList.add("colorRed"));
//! Запись фильма в localStorage
f=[...f,m],console.log("localStorageQueue:",f),//!
localStorage.setItem("queue",JSON.stringify(f)),e(n).Notify.success(`Фильм ${m.title||m.name} добавлен в QUEUE`,{timeout:3500}),
//! Смена названия (textContent) кнопки на "DELETE FROM QUEUE"
i.queueModal.textContent="DELETE FROM QUEUE",i.queueModal.classList.contains("colorGreen")&&i.queueModal.classList.remove("colorGreen"),i.queueModal.classList.contains("colorRed")||i.queueModal.classList.add("colorRed"),console.log("textQueuedModal ==> конец:",o)}else"DELETE FROM QUEUE"===o&&(f=f.filter((e=>e.id!==m.id)),localStorage.setItem("queue",JSON.stringify(f)),console.log("Фильм удален из QUEUE"),e(n).Notify.info(`Фильм ${m.title||m.name} удален из QUEUE`,{timeout:3500}),i.queueModal.textContent="ADD TO QUEUE",i.queueModal.classList.contains("colorRed")&&i.queueModal.classList.remove("colorRed"),i.queueModal.classList.contains("colorGreen")||i.queueModal.classList.add("colorGreen"),"queue"===M&&(console.log("currentPage",M),//!
v(),
//! Очищаем контейнер:
w(),D(f)))}function h(){console.log("Вешаю слушателя на кнопку my-library.js==>WATCHED"),//!
...
function w(){i.moviesCards.innerHTML=""}
//! ++++++++++++++ Ф-ция, к-рая очищает контейнер МОДАЛКИ: ++++++++++++++
function p(){i.InfoMovie.innerHTML=""}
//! ++++++++++++++ Ф-ция, к-рая  прверяет results на пустой массив: ++++++++++++++
//! +++++++++++++++++++++++ Функции для МОДАЛКИ +++++++++++++++++++++++++++
function v(){window.removeEventListener("keydown",q),document.body.classList.remove("show-modal"),
//! Очищаем контейнер МОДАЛКИ:
p()}function q(e){"Escape"===e.code&&v()}
...
//# sourceMappingURL=my-library.3d7f5dfb.js.map
