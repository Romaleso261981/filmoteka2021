!function(){function e(e){return e&&e.__esModule?e.default:e}var t="undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:"undefined"!=typeof window?window:"undefined"!=typeof global?global:{},o={},a={},n=t.parcelRequirea6b3;null==n&&((n=function(e){if(e in o)return o[e].exports;if(e in a){var t=a[e];delete a[e];var n={id:e,exports:{}};return o[e]=n,t.call(n.exports,n,n.exports),n.exports}var l=new Error("Cannot find module '"+e+"'");throw l.code="MODULE_NOT_FOUND",l}).register=function(e,t){a[e]=t},t.parcelRequirea6b3=n);var l=n("iU1Pc"),d=n("g2eb2"),c=n("gp4J8");
...
s.myLibraryBtn.addEventListener("click",M),
...
d.default.startSpinner(),"watched"===u&&(r=g.find((e=>e.id===i)));"queue"===u&&(r=y.find((e=>e.id===i)));
//! ==> Открываем модалку
...
s.watchedModal.addEventListener("click",E),s.queueModal.addEventListener("click",v),
//! Вызываем БЛОК ЛОГИКИ работы кнопок <ADD TO WATCHED> и <ADD TO QUEUE>
(0,c.default)(u,s,r,g,y);
//! +++++++++++++++++++++++++++++ Markup infoFilm ++++++++++++++++++++++++++++++++++++++++++++++
var a}
...
s.watchedHeader.addEventListener("click",M),s.queueHeader.addEventListener("click",(function(){var e;
...
//! Очищаем контейнер:
p();const t=null!==(e=JSON.parse(localStorage.getItem("queue")))&&void 0!==e?e:[];console.log("results:",t),//!
//! Рисование интерфейса
L(t)}
...
let g=null!==(m=JSON.parse(localStorage.getItem("watched")))&&void 0!==m?m:[];var f;let y=null!==(f=JSON.parse(localStorage.getItem("queue")))&&void 0!==f?f:[];
//!!!!!! Загрузка популярных фильмов на страницу MY LIDRARY
function E(){console.log("Вешаю слушателя на кнопку ADD TO WATCHED в МОДАЛКЕ"),//!
...
//! Блокировка повторной записи фильма в localStorage (ВРЕМЕННО)
if(y.find((e=>e.id===r.id)))return e(l).Notify.warning(`Фильм ${r.title||r.name} уже есть в QUEUE`,{timeout:3500}),s.queueModal.textContent="DELETE FROM QUEUE",s.queueModal.classList.contains("colorGreen")&&s.queueModal.classList.remove("colorGreen"),void(s.queueModal.classList.contains("colorRed")||s.queueModal.classList.add("colorRed"));
//! Запись фильма в localStorage
y=[...y,r],console.log("localStorageQueue:",y),//!
localStorage.setItem("queue",JSON.stringify(y)),e(l).Notify.success(`Фильм ${r.title||r.name} добавлен в QUEUE`,{timeout:3500}),
//! Смена названия (textContent) кнопки на "DELETE FROM QUEUE"
s.queueModal.textContent="DELETE FROM QUEUE",s.queueModal.classList.contains("colorGreen")&&s.queueModal.classList.remove("colorGreen"),s.queueModal.classList.contains("colorRed")||s.queueModal.classList.add("colorRed"),console.log("textQueuedModal ==> конец:",t)}else"DELETE FROM QUEUE"===t&&(y=y.filter((e=>e.id!==r.id)),localStorage.setItem("queue",JSON.stringify(y)),console.log("Фильм удален из QUEUE"),e(l).Notify.info(`Фильм ${r.title||r.name} удален из QUEUE`,{timeout:3500}),s.queueModal.textContent="ADD TO QUEUE",s.queueModal.classList.contains("colorRed")&&s.queueModal.classList.remove("colorRed"),s.queueModal.classList.contains("colorGreen")||s.queueModal.classList.add("colorGreen"),"queue"===u&&(console.log("currentPage",u),//!
h(),
//! Очищаем контейнер:
p(),L(y)))}function M(){var e;
//! Перезаписываем в локальную переменную (results) значение всего (localStorage)
...
function p(){s.moviesCards.innerHTML=""}
//! ++++++++++++++ Ф-ция, к-рая очищает контейнер МОДАЛКИ: ++++++++++++++
//! ++++++++++++++ Ф-ция, к-рая  прверяет results на пустой массив: ++++++++++++++
//! +++++++++++++++++++++++ Функции для МОДАЛКИ +++++++++++++++++++++++++++
function h(){window.removeEventListener("keydown",w),document.body.classList.remove("show-modal"),s.InfoMovie.innerHTML=""}function w(e){"Escape"===e.code&&h()}
