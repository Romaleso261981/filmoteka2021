...
c.myLibraryBtn.addEventListener("click",L),
...
c.watchedHeader.addEventListener("click",L),c.queueHeader.addEventListener("click",(function(){var e;
...
//! Очищаем контейнер:
h();const o=null!==(e=JSON.parse(localStorage.getItem("queue")))&&void 0!==e?e:[];console.log("results:",o),//!
//! Рисование интерфейса
q(o)}
...
let m=null!==(u=JSON.parse(localStorage.getItem("watched")))&&void 0!==u?u:[];var M;let g=null!==(M=JSON.parse(localStorage.getItem("queue")))&&void 0!==M?M:[];function f(){console.log("Вешаю слушателя на кнопку ADD TO WATCHED в МОДАЛКЕ"),//!
...
if(m.find((e=>e.id===i.id)))return e(n).Notify.warning(`Фильм ${i.title||i.name} уже есть в WATCHED`,{timeout:3500}),c.watchedModal.textContent="DELETE FROM WATCHED",c.watchedModal.classList.contains("colorGreen")&&c.watchedModal.classList.remove("colorGreen"),void(c.watchedModal.classList.contains("colorRed")||c.watchedModal.classList.add("colorRed"));
//! Запись фильма в localStorage
m=[...m,i],console.log("localStorageWatched:",m),//!
localStorage.setItem("watched",JSON.stringify(m)),e(n).Notify.success(`Фильм ${i.title||i.name} добавлен в WATCHED`,{timeout:3500}),
//! Смена названия (textContent) кнопки на "DELETE FROM WATCHED"
c.watchedModal.textContent="DELETE FROM WATCHED",c.watchedModal.classList.contains("colorGreen")&&c.watchedModal.classList.remove("colorGreen"),c.watchedModal.classList.contains("colorRed")||c.watchedModal.classList.add("colorRed"),console.log("textWatchedModal ==> конец:",o)}else"DELETE FROM WATCHED"===o&&(m=m.filter((e=>e.id!==i.id)),localStorage.setItem("watched",JSON.stringify(m)),console.log("Фильм удален из WATCHED"),//!
e(n).Notify.info(`Фильм ${i.title||i.name} удален из WATCHED`,{timeout:3500}),c.watchedModal.textContent="ADD TO WATCHED",c.watchedModal.classList.contains("colorRed")&&c.watchedModal.classList.remove("colorRed"),c.watchedModal.classList.contains("colorGreen")||c.watchedModal.classList.add("colorGreen"),"watched"===r&&(console.log("currentPage",r),//!
w(),
//! Очищаем контейнер:
h(),q(m)))}
//! +++ Запрос полной информации о фильме для МОДАЛКИ +++
...
//! Блокировка повторной записи фильма в localStorage (ВРЕМЕННО)
if(g.find((e=>e.id===i.id)))return e(n).Notify.warning(`Фильм ${i.title||i.name} уже есть в QUEUE`,{timeout:3500}),c.queueModal.textContent="DELETE FROM QUEUE",c.queueModal.classList.contains("colorGreen")&&c.queueModal.classList.remove("colorGreen"),void(c.queueModal.classList.contains("colorRed")||c.queueModal.classList.add("colorRed"));
//! Запись фильма в localStorage
g=[...g,i],console.log("localStorageQueue:",g),//!
localStorage.setItem("queue",JSON.stringify(g)),e(n).Notify.success(`Фильм ${i.title||i.name} добавлен в QUEUE`,{timeout:3500}),
//! Смена названия (textContent) кнопки на "DELETE FROM QUEUE"
c.queueModal.textContent="DELETE FROM QUEUE",c.queueModal.classList.contains("colorGreen")&&c.queueModal.classList.remove("colorGreen"),c.queueModal.classList.contains("colorRed")||c.queueModal.classList.add("colorRed"),console.log("textQueuedModal ==> конец:",o)}else"DELETE FROM QUEUE"===o&&(g=g.filter((e=>e.id!==i.id)),localStorage.setItem("queue",JSON.stringify(g)),console.log("Фильм удален из QUEUE"),e(n).Notify.info(`Фильм ${i.title||i.name} удален из QUEUE`,{timeout:3500}),c.queueModal.textContent="ADD TO QUEUE",c.queueModal.classList.contains("colorRed")&&c.queueModal.classList.remove("colorRed"),c.queueModal.classList.contains("colorGreen")||c.queueModal.classList.add("colorGreen"),"queue"===r&&(console.log("currentPage",r),//!
w(),
//! Очищаем контейнер:
h(),q(g)))}function L(){var e;
//! Перезаписываем в локальную переменную (results) значение всего (localStorage)
...
//! Кнопка LOAD MORE => показываем и отключаем
//! Очищаем контейнер:
h();const o=null!==(e=JSON.parse(localStorage.getItem("watched")))&&void 0!==e?e:[];console.log("results:",o),//!
//! Рисование интерфейса
q(o)}
//! ++++++++++++++ Ф-ция, к-рая получает id жанра и возвращает тип жанра ++++++++++++++
//! ++++++++++++++ Ф-ция, к-рая очищает контейнер при новом вводе данных в input form: ++++++++++++++
function h(){c.moviesCards.innerHTML=""}
//! ++++++++++++++ Ф-ция, к-рая очищает контейнер МОДАЛКИ: ++++++++++++++
function v(){c.InfoMovie.innerHTML=""}
//! ++++++++++++++ Ф-ция, к-рая  прверяет results на пустой массив: ++++++++++++++
//! +++++++++++++++++++++++ Функции для МОДАЛКИ +++++++++++++++++++++++++++
function w(){window.removeEventListener("keydown",y),document.body.classList.remove("show-modal"),
//! Очищаем контейнер МОДАЛКИ:
v()}function y(e){"Escape"===e.code&&w()}
...
//! --------------------------------------------------------------------------------------------
function(e){return console.log("results:",e),e.map((({id:e,poster_path:o,title:t,name:a,genres:l,first_air_date:n,release_date:d,vote_average:c})=>{const s=l.map((e=>e.name)).join(", "),i=(n||d||"???? - ?? - ??").substr(0,4),r=c.toFixed(1);let u=t;t&&(u=t.toUpperCase());let m=a;if(a){a.toUpperCase();//!!! тут ошибка сделана СПЕЦИАЛЬНО!!!
...
//# sourceMappingURL=my-library.97403e86.js.map
