...
i.myLibraryBtn.addEventListener("click",h),
...
i.closeModalBtn.addEventListener("click",y),i.backdrop.addEventListener("click",(function(e){e.currentTarget===e.target&&y()})),
...
i.watchedHeader.addEventListener("click",h),i.queueHeader.addEventListener("click",(function(){var e;
...
//! Очищаем контейнер:
p();const o=null!==(e=JSON.parse(localStorage.getItem("queue")))&&void 0!==e?e:[];console.log("results:",o),//!
//! Рисование интерфейса 
b(o)}
...
let E=null!==(g=JSON.parse(localStorage.getItem("watched")))&&void 0!==g?g:[];var f;let L=null!==(f=JSON.parse(localStorage.getItem("queue")))&&void 0!==f?f:[];function w(){console.log("Вешаю слушателя на кнопку ADD TO WATCHED в МОДАЛКЕ"),//!
...
if(E.find((e=>e.id===m.id)))return e(n).Notify.warning(`Фильм ${m.title||m.name} уже есть в WATCHED`,{timeout:3500}),i.watchedModal.textContent="DELETE FROM WATCHED",i.watchedModal.classList.contains("colorGreen")&&i.watchedModal.classList.remove("colorGreen"),void(i.watchedModal.classList.contains("colorRed")||i.watchedModal.classList.add("colorRed"));
//! Запись фильма в localStorage
E=[...E,m],console.log("localStorageWatched:",E),//!
localStorage.setItem("watched",JSON.stringify(E)),e(n).Notify.success(`Фильм ${m.title||m.name} добавлен в WATCHED`,{timeout:3500}),
//! Смена названия (textContent) кнопки на "DELETE FROM WATCHED"
i.watchedModal.textContent="DELETE FROM WATCHED",i.watchedModal.classList.contains("colorGreen")&&i.watchedModal.classList.remove("colorGreen"),i.watchedModal.classList.contains("colorRed")||i.watchedModal.classList.add("colorRed"),console.log("textWatchedModal ==> конец:",o)}else"DELETE FROM WATCHED"===o&&(E=E.filter((e=>e.id!==m.id)),localStorage.setItem("watched",JSON.stringify(E)),console.log("Фильм удален из WATCHED"),//!
...
p(),b(E)))}
//! +++ Запрос полной информации о фильме для МОДАЛКИ +++
function v(){console.log("Вешаю слушателя на кнопку ADD TO QUEUE в МОДАЛКЕ"),//!
console.log("infoFilm:",m),//!
console.log("infoFilm.id:",m.id);//!
const o=i.queueModal.textContent;//!
if(console.log("textQueuedModal ==> начало:",o),"ADD TO QUEUE"===o){
//! Блокировка повторной записи фильма в localStorage (ВРЕМЕННО)
if(L.find((e=>e.id===m.id)))return e(n).Notify.warning(`Фильм ${m.title||m.name} уже есть в QUEUE`,{timeout:3500}),i.queueModal.textContent="DELETE FROM QUEUE",i.queueModal.classList.contains("colorGreen")&&i.queueModal.classList.remove("colorGreen"),void(i.queueModal.classList.contains("colorRed")||i.queueModal.classList.add("colorRed"));
//! Запись фильма в localStorage
L=[...L,m],console.log("localStorageQueue:",L),//!
localStorage.setItem("queue",JSON.stringify(L)),e(n).Notify.success(`Фильм ${m.title||m.name} добавлен в QUEUE`,{timeout:3500}),
//! Смена названия (textContent) кнопки на "DELETE FROM QUEUE"
i.queueModal.textContent="DELETE FROM QUEUE",i.queueModal.classList.contains("colorGreen")&&i.queueModal.classList.remove("colorGreen"),i.queueModal.classList.contains("colorRed")||i.queueModal.classList.add("colorRed"),console.log("textQueuedModal ==> конец:",o)}else"DELETE FROM QUEUE"===o&&(L=L.filter((e=>e.id!==m.id)),localStorage.setItem("queue",JSON.stringify(L)),console.log("Фильм удален из QUEUE"),e(n).Notify.info(`Фильм ${m.title||m.name} удален из QUEUE`,{timeout:3500}),i.queueModal.textContent="ADD TO QUEUE",i.queueModal.classList.contains("colorRed")&&i.queueModal.classList.remove("colorRed"),i.queueModal.classList.contains("colorGreen")||i.queueModal.classList.add("colorGreen"),"queue"===M&&(console.log("currentPage",M),//!
y(),
//! Очищаем контейнер:
p(),b(L)))}function h(){var e;
//! Перезаписываем в локальную переменную (results) значение всего (localStorage)
...
//! Кнопка LOAD MORE => показываем и отключаем
//! Очищаем контейнер:
p();const o=null!==(e=JSON.parse(localStorage.getItem("watched")))&&void 0!==e?e:[];console.log("results:",o),//!
//! Рисование интерфейса 
b(o)}
//! ++++++++++++++ Ф-ция, к-рая получает id жанра и возвращает тип жанра ++++++++++++++
//! ++++++++++++++ Ф-ция, к-рая очищает контейнер при новом вводе данных в input form: ++++++++++++++
function p(){i.moviesCards.innerHTML=""}
//! ++++++++++++++ Ф-ция, к-рая очищает контейнер МОДАЛКИ: ++++++++++++++
function q(){i.InfoMovie.innerHTML=""}
//! ++++++++++++++ Ф-ция, к-рая  прверяет results на пустой массив: ++++++++++++++
//! +++++++++++++++++++++++ Функции для МОДАЛКИ +++++++++++++++++++++++++++
function y(){window.removeEventListener("keydown",T),document.body.classList.remove("show-modal"),
//! Очищаем контейнер МОДАЛКИ:
q()}function T(e){"Escape"===e.code&&y()}
...
//! --------------------------------------------------------------------------------------------
function(e){return console.log("results:",e),e.map((({id:e,poster_path:o,title:t,name:a,genres:l,first_air_date:n,release_date:d,vote_average:s})=>{const c=l.map((e=>e.name)).join(", "),i=(n||d||"???? - ?? - ??").substr(0,4),r=s.toFixed(1);let u=t;t&&(u=t.toUpperCase());let m=a;if(a){a.toUpperCase();//!!! тут ошибка сделана СПЕЦИАЛЬНО!!!
}return`\n                <li key=${e}>\n                    <img src="https://image.tmdb.org/t/p/w780${o}" alt="${t||a}" />\n\n                    <div>\n                        <h2>${u||m}</h2>\n                        <h3>${c} &nbsp|&nbsp ${i}&nbsp &nbsp${r}</h3>\n                    </div>\n                </li>\n                `})).join("")}(e))}(()=>{const e={openModalTeamLink:document.querySelector(".team-link"),closeModalTeamBtn:document.querySelector(".team-close-btn"),modalTeam:document.querySelector(".backdrop-team"),body:document.querySelector("body")},{openModalTeamLink:o,closeModalTeamBtn:t,modalTeam:a,body:l}=e;function n(e){a.classList.toggle("is-hidden"),l.classList.toggle("no-scroll"),window.removeEventListener("keydown",d)}function d(e){"Escape"===e.key&&n()}o.addEventListener("click",(function(e){window.addEventListener("keydown",d),a.classList.toggle("is-hidden"),l.classList.toggle("no-scroll")})),t.addEventListener("click",n),a.addEventListener("click",(function(e){e.currentTarget===e.target&&n()}))})()}();
//# sourceMappingURL=my-library.518722bc.js.map
